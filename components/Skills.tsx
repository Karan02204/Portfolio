...
export default function Skills() {
  ...
  return (
    <section id="skills" ref={ref} className="py-32 px-6 md:px-20 relative" style={{ background: "var(--background)" }}>
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-20">
          {categories.map((cat, ci) => (
            ...
            >
              <p className="text-xs tracking-[0.2em] uppercase mb-4" style={{ color: cat.color, fontFamily: "var(--font-inconsolata)" }}>
                {cat.label}
              </p>
        ...
        <motion.div
          ...
          className="mb-20"
        >
          <p className="text-xs tracking-[0.3em] uppercase mb-8" style={{ color: "rgba(255,255,255,0.4)", fontFamily: "var(--font-inconsolata)" }}>
            Key Proficiencies
          </p>
          <div className="flex flex-col gap-4">
            {proficiencies.map((item, i) => (
              <div key={item.name} className="flex items-center gap-4">
                <span className="w-28 text-sm text-right" style={{ color: "rgba(255,255,255,0.6)", fontFamily: "var(--font-inconsolata)" }}>
                  {item.name}
                </span>
                ...
                <span className="text-xs w-8" style={{ color: "#f48b34", fontFamily: "var(--font-inconsolata)" }}>
                  {item.pct}%
                </span>
              </div>
            ))}
        ...
        <div className="overflow-hidden border-y py-5" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <div className="marquee-track flex gap-10" style={{ width: "max-content" }}>
            {[...allSkills, ...allSkills].map((s, i) => (
              <span key={i} className="whitespace-nowrap text-sm" style={{ color: "rgba(255,255,255,0.3)", fontFamily: "var(--font-inconsolata)" }}>
